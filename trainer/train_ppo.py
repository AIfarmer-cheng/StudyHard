...
class CriticModel(MokioMindForCausalLM):
    def __init__(self, params):
        ...
        # 价值头，对actor输出的每个词打分(比如说[2,100,512]->[2,100,1])，2句话，每句话100个字
        # critic=actor+价值头
        self.value_head = nn.Linear(params.hidden_size, 1)

    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
        # 隐藏层状态，output[0]是模型最后的”想法“，然后再优雅化一下
        hidden_states = self.model.norm(outputs[0]) # (插一嘴，想要的到每个词的概率，先通过Linear投射到所有词的数量，再通过softmax)
...
def calculate_rewards(prompts, responses, reward_model, reward_tokenizer):
    def reasoning_model_reward(rewards):
        ...
        for match_pattern, match_pattern2 in zip(matches_pattern, matches_pattern2):
            if match_pattern:
                format_rewards.append(0.5) # 格式如果匹配思考回答，奖励0.5分
            elif match_pattern2:
...
def ppo_train_epoch(
    ...
    # 切换actor和critic模型到训练模式，他俩是需要跟新参数的
    actor_model.train()
    critic_model.train()
